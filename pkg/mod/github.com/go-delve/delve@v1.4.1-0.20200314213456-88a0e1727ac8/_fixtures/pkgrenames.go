...
func main() {
	...
	var amap interface{} = map[pkg1.BadExpr]pkg2.Request{{2, 3}: {Method: "othermethod"}}
	var amap2 interface{} = &map[pkg1.BadExpr]pkg2.Request{{2, 3}: {Method: "othermethod"}}
	var dir0someType interface{} = &pkg.SomeType{3}
	var dir1someType interface{} = dir1pkg.SomeType{1, 2}
	var amap3 interface{} = map[pkg.SomeType]dir1pkg.SomeType{{4}: {5, 6}}
	var anarray interface{} = [2]pkg.SomeType{{1}, {2}}
	var achan interface{} = make(chan pkg.SomeType)
	var aslice interface{} = []pkg.SomeType{{3}, {4}}
